...
import { AVPlayerState, CommonConstants, VideoDataType } from "@bundle:com.example.AVPlayerLongVideo/entry/ets/common/constants/CommonConstants";
import type { VideoData } from '../model/VideoData';
import { AvSessionController } from "@bundle:com.example.AVPlayerLongVideo/entry/ets/controller/AvSessionController";
...
export class AvPlayerController {
    ...
    surfaceID: string = '';
    @Track
    isPlaying: boolean = false; // Whether the video is playing
    @Track
    ...
    @Track
    duration: number = 0; // Current play time
    @Track
    durationTime: number = 0; // Video duration
    @Track
            ...
            switch (state) {
                ...
                case 'initialized': // This status is reported after the playback source is set on the AVPlayer.
                    ...
                    try {
                        ...
                        Logger.error(TAG, `setPlaybackStrategy failed. Cause code: ${exception.code}, message: ${exception.message}`);
                    }
    ...
    public async setAvSessionListener() {
        ...
        try {
            ...
            Logger.error(TAG, `Invoke setAvSessionListener failed, code is ${exception.code}, message is ${exception.message}`);
        }
    }
    ...
    subtitleUpdateFunction(): void {
        try {
            if (this.avPlayer) {
                ...
                this.avPlayer.on('subtitleUpdate', (info: media.SubtitleInfo) => {
                    if (info) {
                        ...
                        Logger.info(TAG, `subtitleUpdate info: text:${text}, startTime:${startTime}, duration:${duration}`);
                    }
                    else {
                        this.currentCaption = '';
            ...
            Logger.error(TAG, `subtitleUpdateFunction failed, code is ${err.code}, message is ${err.message}`);
        }
    }
    async languageChange(languageSelect: number = 0): Promise<void> {
        if (this.avPlayer) {
            try {
                ...
                Logger.error(TAG, `languageChange failed, code is ${err.code}, message is ${err.message}`);
            }
        }
    }
